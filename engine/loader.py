...
def load(db_path):
    ...
    # Check which neuron columns exist (backwards compat for old DBs)
...
def save(brain):
    ...
    # Check if DB has neuromodulator columns
    col_info = conn.execute("PRAGMA table_info(neurons)").fetchall()
    has_neuromod = 'dopamine_sens' in {c[1] for c in col_info}

    for n in brain['neurons']:
        if has_neuromod:
            conn.execute(
                "UPDATE neurons SET v=?, u=?, last_spike=?, a=?, b=?, c=?, d=?, "
                "dopamine_sens=?, excitability=?, activity_trace=? WHERE id=?",
                (n['v'], n['u'], n['last_spike'], n['a'], n['b'], n['c'], n['d'],
                 n.get('dopamine_sens', 0.0), n.get('excitability', 0.0),
                 n.get('activity_trace', 0.0), n['id'])
            )
        else:
            conn.execute(
