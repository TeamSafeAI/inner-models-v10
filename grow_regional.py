...
def main():
    ...
    # 4. General stats
    total_syn = len(synapses)
    unique_pairs = len(pair_counts)
    mean_multi = np.mean(list(pair_counts.values())) if pair_counts else 0
    print(f"\n  Total synapses: {total_syn}, Unique pairs: {unique_pairs}, Mean multiplicity: {mean_multi:.2f}")
