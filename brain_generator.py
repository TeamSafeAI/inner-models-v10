...
def generate(recipe):
    ...
    # --- Post-processing: parameter jitter + neuromodulator sensitivity ---
    # Izhikevich 2003 recipe: randomize c,d within type-appropriate ranges
    # This creates intrinsic diversity so identical components behave differently
    jitter_std = wiring.get('param_jitter', 0.1)  # 0 = no jitter, 0.1 = 10%
    if jitter_std > 0:
        rng = random.Random(seed + 7777)
        n_jittered = 0
        rows = conn.execute("SELECT id, neuron_type, a, b, c, d FROM neurons").fetchall()
        for row in rows:
            nid, ntype, a, b, c, d = row
            # Skip neurons with explicit overrides (a=0 bistable, etc.)
            if abs(a) < 1e-6:
                continue  # intentionally zeroed, don't jitter
            # Jitter: Gaussian noise scaled by param magnitude
            ja = a * (1.0 + rng.gauss(0, jitter_std * 0.5))  # a is small, gentle jitter
            jb = b * (1.0 + rng.gauss(0, jitter_std * 0.5))
            jc = c + rng.gauss(0, abs(c) * jitter_std * 0.1)  # c is negative, additive
            jd = max(0, d * (1.0 + rng.gauss(0, jitter_std)))  # d can vary more
            conn.execute("UPDATE neurons SET a=?, b=?, c=?, d=? WHERE id=?",
                         (ja, jb, jc, jd, nid))
            n_jittered += 1
        print(f"  Parameter jitter: {n_jittered} neurons (std={jitter_std})")

    # Assign dopamine sensitivity to emotional/valence neurons
    # D1-like (+): reward excites. D2-like (-): reward inhibits.
    # Only assign to neurons that are sources of reward_plastic synapses.
    try:
        col_check = conn.execute("PRAGMA table_info(neurons)").fetchall()
        if 'dopamine_sens' in {c[1] for c in col_check}:
            rng_da = random.Random(seed + 8888)
            reward_sources = conn.execute(
                "SELECT DISTINCT source FROM synapses WHERE synapse_type='reward_plastic'"
            ).fetchall()
            reward_targets = conn.execute(
                "SELECT DISTINCT target FROM synapses WHERE synapse_type='reward_plastic'"
            ).fetchall()
            n_da = 0
            # Sources (emotional neurons): ~40% D1, ~40% D2, ~20% mixed
            for (nid,) in reward_sources:
                r = rng_da.random()
                if r < 0.4:
                    sens = rng_da.uniform(0.3, 1.0)   # D1-like
                elif r < 0.8:
                    sens = rng_da.uniform(-1.0, -0.3)  # D2-like
                else:
                    sens = rng_da.uniform(-0.2, 0.2)   # salience/mixed
                conn.execute("UPDATE neurons SET dopamine_sens=? WHERE id=?", (sens, nid))
                n_da += 1
            # Targets (decision neurons): lighter sensitivity
            for (nid,) in reward_targets:
                sens = rng_da.uniform(-0.3, 0.3)  # mild, varied
                conn.execute("UPDATE neurons SET dopamine_sens=? WHERE id=?", (sens, nid))
                n_da += 1
            if n_da > 0:
                print(f"  Dopamine sensitivity: {n_da} neurons assigned (D1/D2/mixed)")
    except Exception:
        pass  # old schema without dopamine_sens column
