...
def get_frame(brain, neuron_types, tick, fired, motor_fire):
    ...
    return {
        ...
        'signals': {
            'surprise': round(float(getattr(brain, 'surprise', 0)), 4),
...
async def simulation_loop(websocket, brain, brain_data, neuron_types,
    ...
    # Electrode index arrays (fixed — new neurons aren't electrodes)
    ...
    touch_size = brain.n  # tracks current size for touch injection

    # Growth state
    growth_rng = np.random.RandomState(42)
    frame_count = 0
    growth_interval = 15  # frames between growth checks (~0.5s)
    ...
    while running:
        ...
        if not paused:
            cur_n = brain.n  # may change if growth happened

            # Build external current: tonic base + population-encoded sensory input
            I_ext = np.full(cur_n, args.tonic)
            ...
            # Inject into electrode neurons (indices stay valid — they're original neurons)
            ...
            # Touch injection (resize if brain grew)
            if touch_size < cur_n:
                touch_size = cur_n
            # Touch is applied via direct 'touch' action per-frame, decays naturally
            ...
            # Dynamic growth (every N frames, if enabled)
            frame_count += 1
            growth_stats = None
            if args.growth and frame_count % growth_interval == 0:
                growth_stats = brain.dynamic_growth(rng=growth_rng)
                if growth_stats['neurons_born'] > 0:
                    # Send growth event with new neuron positions for viewer
                    new_neurons = []
                    for ni in range(cur_n, brain.n):
                        nn = brain.neurons[ni]
                        nt = nn['type']
                        color = NEURON_TYPE_COLORS.get(nt, [0.5, 0.5, 0.5])
                        new_neurons.append({
                    ...
                    await websocket.send(json.dumps({
                        ...
                        'n_neurons': brain.n,
                        'n_synapses': len(brain.synapses),
                        'born': growth_stats['neurons_born'],
                        'wired': growth_stats['synapses_added'],
                    }))
                    # Update neuron_types list for frame building
                    for ni in range(cur_n, brain.n):
                        neuron_types.append(brain.neurons[ni]['type'])
            ...
            frame['n_neurons'] = brain.n
            frame['n_synapses'] = len(brain.synapses)
...
async def main():
    ...
    p.add_argument('--growth', action='store_true', help='Enable runtime neurogenesis (signal-driven)')
    args = p.parse_args()
